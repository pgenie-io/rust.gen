...
pub type CreatePlaylistResult = CreatePlaylistResultRow;
...
/// SQL query string.
pub const SQL: &str = "INSERT INTO playlists (name, description, user_id, created_at)\nVALUES ($2, $3, $1, NOW())\nRETURNING id, name, created_at";
...
pub async fn query(
    ...
    let row = client.query_one(SQL, &[&params.name as &(dyn postgres_types::ToSql + Sync), &params.description as &(dyn postgres_types::ToSql + Sync), &params.user_id as &(dyn postgres_types::ToSql + Sync)]).await?;
    Ok(row.into())
}
